...
#for(i in 1:rep){
  ...
  #ppyseerdir <- paste("Pvalue", "/LM.csv",sep = '')
...
g = ggplot(df) + geom_line(aes(x = FPR, y = TPR, colour = model)) +
  labs(title = 'N = 15, M = 100') + theme_minimal() +
...
write.csv(c(model.lm$auc, model.lmm$auc, model.tr$auc, model.pyseer$auc), file = 'AUC0.csv')
write.csv(c(lmTPR, lmmTPR, lmmTTPR, pyseerTPR), file = 'TPR0.csv')
